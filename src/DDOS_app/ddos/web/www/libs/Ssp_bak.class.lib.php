...
class SSP {
         ...
         */
        static function filter ( $request, $columns, &$bindings, $isJoin = false, $joinQuery = NULL )
        {
                $globalSearch = array();
                $columnSearch = array();
                $dtColumns = SSP::pluck( $columns, 'dt' );
        ...
        static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL )
        {
                ...
                if($joinQuery){
                 $psql = "SELECT ".implode(", ", SSP::pluck($columns, 'db', $joinQuery)).", count(*) OVER() AS total_count
                         FROM $table
                         $joinQuery
                         $where
                         $order
